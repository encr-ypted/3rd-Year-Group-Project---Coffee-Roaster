...
def get_latest_log_file():  
    files = glob.glob("logs/roast_*.csv")  # get all roast csv files inside logss folder
...
def get_latest_row():  
    ...
    try:  
        with open(log_file, "r") as file:  
            rows = list(csv.DictReader(file))  #reads csv rows as dictionaries using column names

        if len(rows) == 0:  
            raise RuntimeError("No CSV rows")  

        latest = rows[-1]  #take the last row

        return {  
            "type": "telemetry",  
            "timestamp": float(latest.get("time_s", 0)),  
            "temp": float(latest.get("temp_c", 0)),  
