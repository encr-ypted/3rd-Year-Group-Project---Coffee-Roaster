...
ENABLE_AI_GRAYSCALE = True
GRAYSCALE_STATE_FILE = "grayscale_state.json"
GRAYSCALE_STALE_AFTER_S = 3.0
GRAYSCALE_DONE_THRESHOLDS = {
    "light": 120.0,
    "medium": 120.0,
    "medium-dark": 110.0,
    "dark": 105.0,
...
DEFAULT_GRAYSCALE_DONE_THRESHOLD = 115.0
GRAYSCALE_DONE_HOLD_S = 5.0
...
MODEL_B_MIN = 0.002
MODEL_B_MAX = 0.0130
MODEL_B_ADAPT_INTERVAL_S = 10.0
MODEL_B_ADAPT_STEP = 0.00017
MODEL_B_ERROR_THRESHOLD_C = 1.5

PREDICTION_HORIZON = 68
MPC_TANGENT_WEIGHT = 0.8
DUTY_STEP = 1
...
gray_below_since = None
roast_done_by_grayscale = False
latest_mean_grayscale = None
latest_grayscale_age_s = None
...
def read_command():
    ...
    if not os.path.exists(COMMAND_FILE):
        return "IDLE", current_target_c, current_profile_id
    ...
    if "target_c" in payload:
        try:
            current_target_c = float(payload["target_c"])
        except Exception:
            pass

    if "profile_id" in payload:
        current_profile_id = str(payload["profile_id"])

    return command, current_target_c, current_profile_id
...
def mpc_reference_setpoint(start_temp_c, target_c, elapsed_s, future_step_s, midpoint_min, steepness):
    curved = setpoint_curve(
        start_temp_c,
        target_c,
        elapsed_s + future_step_s,
        midpoint_min,
        steepness
    )

    current = setpoint_curve(
        start_temp_c,
        target_c,
        elapsed_s,
        midpoint_min,
    ...
    slope_c_per_s = next_point - current
    tangent = current + slope_c_per_s * future_step_s

    blended = ((1.0 - MPC_TANGENT_WEIGHT) * curved) + (MPC_TANGENT_WEIGHT * tangent)

    # Do not let the MPC reference ask for a temperature above the final target.
    return min(blended, target_c)
    ...
    for duty in range(0, 101, DUTY_STEP):
        ...
        for step in range(PREDICTION_HORIZON):
            ...
            if elapsed_s is not None and roast_start_temp_c is not None:
                future_setpoint = mpc_reference_setpoint(
                    roast_start_temp_c,
                    target_c,
                    elapsed_s,
                    step,
            ...
            if predicted_temp > future_setpoint + 10:
...
def grayscale_done_threshold_for_profile(profile_id):
    return GRAYSCALE_DONE_THRESHOLDS.get(
        profile_id,
        DEFAULT_GRAYSCALE_DONE_THRESHOLD
    )



def read_grayscale_from_worker():
    """Read grayscale written by grayscale_worker.py.

    This avoids importing or running the camera/IMX500 stack in the roast-control
    process. If the camera process segfaults, this process keeps roasting.
    """
    global latest_mean_grayscale
    global latest_grayscale_age_s

    if not ENABLE_AI_GRAYSCALE:
        latest_mean_grayscale = None
        latest_grayscale_age_s = None
        return None

    try:
        with open(GRAYSCALE_STATE_FILE, "r") as file:
            payload = json.load(file)

        ts = float(payload.get("timestamp", 0.0))
        age_s = time.time() - ts
        latest_grayscale_age_s = age_s

        if age_s > GRAYSCALE_STALE_AFTER_S:
            latest_mean_grayscale = None
            return None

        value = payload.get("mean_grayscale", None)
        if value in [None, "", "NaN"]:
            latest_mean_grayscale = None
            return None

        latest_mean_grayscale = float(value)
        return latest_mean_grayscale
...
try:
    ...
    while True:
        ...
        command, target_c, profile_id = read_command()
        ...
        try:
            ...
            # Read grayscale from a separate worker process. If the camera/AI stack
            # crashes or segfaults, this control loop keeps roasting.
            read_grayscale_from_worker()
            ...
            if preheat_ready and preheat_ready_temp_c is not None:
                if not bean_drop_started:
                    ...
                    if temp_c >= bean_drop_lowest_temp_c + BEAN_DROP_RISE_CONFIRM_C:
                        command = "RUN"
                        write_command("RUN", target_c=target_c, profile_id=profile_id)
                        start_time = time.time()
                        elapsed = 0.0
            ...
            if command == "IDLE":
                ...
                if bean_drop_started:
                    # Bean drop causes a sharp temperature crash.
                    # Do not fight it with heat while waiting for the bottom/rise confirmation.
                    heater_output = 0.0
                    previous_heater_output = 0.0
                    setpoint_c = temp_c
                    state = "BEAN_DROP_DETECTED_WAITING_FOR_BOTTOM"
                ...
                if latest_mean_grayscale is not None:
                    grayscale_threshold = grayscale_done_threshold_for_profile(profile_id)

                    if latest_mean_grayscale < grayscale_threshold:
                        if gray_below_since is None:
                            gray_below_since = time.monotonic()
                        elif time.monotonic() - gray_below_since >= GRAYSCALE_DONE_HOLD_S:
                            # Notify only. Do NOT stop the roast, change heater output, or change state.
                            roast_done_by_grayscale = True
                    else:
                        gray_below_since = None
                        roast_done_by_grayscale = False
                else:
            ...
            with open(log_file, "a", newline="") as file:
                ...
                writer.writerow([
                    ...
                    heater_output,
                    enable.value,
                    round(latest_mean_grayscale, 2) if latest_mean_grayscale is not None else "",
                    grayscale_done_threshold_for_profile(profile_id),
                    roast_done_by_grayscale,
                    state
                ])
            ...
            print(
                ...
                f"Model B: {active_model_b:.6f} | "
                f"Gray: {latest_mean_grayscale if latest_mean_grayscale is not None else '—'} | "
                f"Gray age: {latest_grayscale_age_s:.1f}s | " if latest_grayscale_age_s is not None else "Gray age: — | "
                f"Gray threshold: {grayscale_done_threshold_for_profile(profile_id):.1f} | "
                f"Done notify: {roast_done_by_grayscale} | {state}"
            )
